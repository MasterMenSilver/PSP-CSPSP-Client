...
void GameStateOnline::Update(float dt)
{
	...
	if (mState == PLAYING) {
		...
		if (mGuiController->IsActive())
		{
			...
			else if (mEngine->GetButtonClick(PSP_CTRL_SELECT)) { //resetround
				char chatstring[32];
				sprintf(chatstring,"/resetround");
				Packet sendpacket = Packet();
				sendpacket.WriteInt8(CHAT);
				sendpacket.WriteChar(chatstring);
				sendpacket.WriteInt8(false);
...
void GameStateOnline::Render() 
{
	...
	if (mIsMapChanging) {
		...
		gFont->DrawShadowedString("[[]] Kick     [^] Ban    [SELECT] Reset Round     [O] Return",SCREEN_WIDTH_2,SCREEN_HEIGHT_F-20,JGETEXT_CENTER);
	}
