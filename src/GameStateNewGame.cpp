...
{
	FILE *file;
	file = fopen("data/Gamemode.cfg", "w"); 
	if (file == NULL) return;
...
void GameStateNewGame::Render()
{
	...
	if(mState == GaM) {
		...
		mRenderer->FillRect(0,0,SCREEN_WIDTH,30,ARGB(175,0,0,0));
											//  ARGB(100,0,0,0)

		gFont->SetColor(ARGB(255,255,244,53));
		gFont->SetScale(1.0f);
		gFont->DrawShadowedString("Gamemode Selection",SCREEN_WIDTH_2,6,JGETEXT_CENTER);
		
		if(GM == TDM) {
			gFont->SetColor(ARGB(255,255,255,255));
			gFont->SetScale(1.0f);
			gFont->DrawShadowedString("Gamemode: TDM",SCREEN_WIDTH_2,80,JGETEXT_CENTER);
		}
		if(GM == CTF) {
			gFont->SetColor(ARGB(255,255,255,255));
			gFont->SetScale(1.0f);
			gFont->DrawShadowedString("Gamemode: CTF",SCREEN_WIDTH_2,80,JGETEXT_CENTER);
		}
		if(GM == FFA) {
			gFont->SetColor(ARGB(255,255,255,255));
			gFont->SetScale(1.0f);
			gFont->DrawShadowedString("Gamemode: FFA",SCREEN_WIDTH_2,80,JGETEXT_CENTER);
		}
